...
class _MyWidgetState extends State<CoopAssetsWidget> {
 ...
 String? loggedInUserId;
  ...
  void _fetchGroupMembers() async {
    ...
    try {
      var  assets_list = await assetController.getGroupAssets(widget.group.id ?? '');
      setState(()  {
        assets = assets_list;
        _isLoading = false;
      });
  ...
  Widget build(BuildContext context) {
    ...
    width = size.width;
    height = size.height;
    return _isLoading ? const Center(child: CircularProgressIndicator()) : assets!.isEmpty ? const Center(child: Text('No assets found')) : ListView.builder(
